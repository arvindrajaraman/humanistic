  ...
  if($fmedia !="NULL"){
    if($fmedia == "external") {
      ...
      if(in_array($ext, $supported_images)){
        //supported image
        $fmedia = "<img class='project-fmedia' src='resources/files/projects/" . $project_id . "/" . $fmedia . "' alt='" . "' width='100%'/>";
      } elseif(in_array($ext, $supported_videos)){
        //supported video
        $fmedia = "<video class='project-fmedia' controls  width='100%'>" .
...
if($no_members){
  ...
  if($request = mysqli_query($conn, $sql)){
    while ($removed = mysqli_fetch_array($request)){
      ...
      $sql2 = "DELETE from project_members where project_id='" . $project_id . "' AND user_id='" . $removed['id'] . "'";
...
$sql = "SELECT users.username, users.id FROM `project_members`,`users` where project_members.user_id = users.id and project_id=" . $project_id . " and users.username IN " . $project_members_sql;
...
if($request = mysqli_query($conn, $sql)){
  while ($added = mysqli_fetch_array($request)){
    ...
    $sql2 = "INSERT into project_members (project_id, user_id, member_role, accepted) values('" . $project_id . "', '" . $added['id'] . "', 'member', '0')";
    $request2 = mysqli_query($conn, $sql2);
...
if($fmedia_flag == 1){
$sql = "UPDATE projects set summary = '" . $summary . "', about = '" . $about . "', tags='" . $tags . "', stage='" . $stage . "', name='" . $project_name . "', fmedia='" . $fmedia . "', public='" . $public_visibility . "' where id='" . $_POST['project-id'] . "'";
}else if($fmedia_flag == -1){
$sql = "UPDATE projects set summary = '" . $summary . "', about = '" . $about . "', tags='" . $tags . "', stage='" . $stage . "', name='" . $project_name . "', public='" . $public_visibility . "' where id='" . $_POST['project-id'] . "'";
}else if($fmedia_flag == 0){
$sql = "UPDATE projects set summary = '" . $summary . "', about = '" . $about . "', tags='" . $tags . "', stage='" . $stage . "', name='" . $project_name . "', fmedia=NULL , public='" . $public_visibility . "' where id='" . $_POST['project-id'] . "'";
}
